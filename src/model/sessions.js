//check if a user is in db, if a logged-in, check session expire time
...
//function to get user-id from session table
function createSession(user_id) {
    ...
    const id = crypto
        ...
        .toString(
            'base64'
        ); /*Generates a strong, long, random string to use as the session ID */
    insert_session.run({
        id,
        user_id,
    }); /*Inserts a new session into the database (including the user ID). Also session.run expects a value and { id, user_id } is passed in as an object*/
